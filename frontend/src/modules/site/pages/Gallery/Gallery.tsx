    ...
    return (<section className="gallery-section">
            <div className="section-content-wrapper">
                ...
                <div className="gallery-type-selector">
                    <select className="gallery-select" value={selectedType} onChange={(e) => setSelectedType(e.target.value)}>
                        <option value="reel">Reels (portret)</option>
                        <option value="landscape">YouTube stil (pejzaž)</option>
                    </select>
                </div>

                {selectedType === "reel" ? (<ReelGallery videos={filteredVideos} onVideoClick={handleVideoClick}/>) : (<LandscapeGallery videos={filteredVideos} onVideoClick={handleVideoClick}/>)}
            </div>

            {activeVideo && (<VideoModal video={activeVideo} onClose={handleCloseModal}/>)}
        </section>);
}
