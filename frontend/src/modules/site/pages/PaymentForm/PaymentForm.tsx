...
export default function PaymentForm() {
    ...
    useEffect(() => {
        api.get("/services")
            ...
            if (serviceIdParam) {
                ...
                if (res.data.some((s: Service) => s.id === id)) {
    ...
    const handlePaymentSuccess = () => {
        ...
        setSuccess(true);
        setErrors({ name: "", companyOIB: "", companyName: "", phoneNumber: "", website: "", nameAndLastName: "", eMail: "" });
        setFiles([]);
    ...
    return (<section className="payment-form-section">
      <div className="section-content-wrapper">
        <h1>Za početak ne treba puno...</h1>

        {success && (<div className="form-success">Vaša narudžba je zaprimljena i plaćena!</div>)}
        ...
        <div className="payment-form">
          ...
          <div className="form-row">
            <div className="form-input-item">
              <input type="text" name="nameAndLastName" placeholder="ime i prezime*" value={formData.nameAndLastName} onChange={handleChange} className={errors.nameAndLastName ? "input-error" : ""}/>
              {errors.nameAndLastName && <span className="field-error">{errors.nameAndLastName}</span>}
            </div>
            <div className="form-input-item">
              <input type="email" name="eMail" placeholder="e-mail*" value={formData.eMail} onChange={handleChange} className={errors.eMail ? "input-error" : ""}/>
          ...
          <div className="form-row">
            <div className="form-input-item">
              <input type="tel" name="phoneNumber" placeholder="broj telefona" value={formData.phoneNumber} onChange={handleChange} className={errors.phoneNumber ? "input-error" : ""}/>
              {errors.phoneNumber && <span className="field-error">{errors.phoneNumber}</span>}
            </div>
            <div className="form-input-item">
              <input type="url" name="website" placeholder="web stranica" value={formData.website} onChange={handleChange} className={errors.website ? "input-error" : ""}/>
              {errors.website && <span className="field-error">{errors.website}</span>}
            </div>
          </div>

          <div className="form-row">
            <textarea className="order-description" name="customerRequest" placeholder="opišite nam tko ste i što nudite svojim klijentima" value={formData.customerRequest} onChange={handleChange}/>
          </div>

          <div className="form-row">
            <div className="upload-wrapper">
              <input ref={fileInputRef} type="file" multiple accept=".pdf,.ppt,.pptx,.doc,.docx,.txt,.jpeg,.jpg,.png,.gif,.zip,.rar" onChange={handleFileInputChange} style={{ display: 'none' }}/>
              <div className={`upload-field ${isDragging ? 'upload-field-dragging' : ''}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={openFileDialog}>
                <span>ovdje povucite svoj logo, fotografije, brošure... ili kliknite za odabir</span>
              </div>
              {files.length > 0 && (<div className="file-pills">
                  {files.map((file, index) => (<div key={`${file.name}-${index}`} className="file-pill">
                      <span className="file-pill-name">{file.name}</span>
                      <span className="file-pill-size">({formatFileSize(file.size)})</span>
                      <button type="button" className="file-pill-remove" onClick={(e) => {
                    e.stopPropagation();
                    removeFile(index);
                }}>
                        ✕
          ...
          <div className="form-row">
            <div className="checkbox-wrapper" onClick={toggleR1} role="button">
              <img className="checkbox-icon" src={formData.isR1Reciept
            ? checkboxCheckedIcon
            : checkboxUncheckedIcon} alt=""/>
          ...
          {formData.isR1Reciept && (<div className="form-row">
              <div className="form-input-item">
                <input type="text" name="companyName" placeholder="naziv tvrtke, obrta, OPG-a...*" value={formData.companyName} onChange={handleChange} className={errors.companyName ? "input-error" : ""}/>
                {errors.companyName && <span className="field-error">{errors.companyName}</span>}
              </div>
              <div className="form-input-item">
                <input type="text" name="companyOIB" placeholder="OIB tvrtke, obrta, OPG-a...*" value={formData.companyOIB} onChange={handleChange} className={errors.companyOIB ? "input-error" : ""}/>
                {errors.companyOIB && <span className="field-error">{errors.companyOIB}</span>}
              </div>
            </div>)}

          <button className="pay-video-button" onClick={onPayButton} disabled={showPaymentModal || !selectedServiceId}>
            {selectedServiceId ? `Plati €${orderAmount.toFixed(2)}` : "Odaberite uslugu iznad"}
          </button>
        </div>

        {showPaymentModal && selectedServiceId && (<StripePaymentModal formData={formData} files={files} serviceId={selectedServiceId} amount={orderAmount} onSuccess={handlePaymentSuccess} onCancel={handlePaymentCancel}/>)}
      </div>
    </section>);
}
